   ...
   try {
       ...
       if (data.length > 0) {
        coursContanaire.innerHTML =''
           data.forEach((cours) => {
           let tags= cours.tags.split(',');
           
            
            coursContanaire.innerHTML += `<article  class="bg-white mt-4 rounded-xl shadow hover:shadow-md transition-shadow duration-300">
<div class="relative">
  <iframe class="w-full h-40 object-cover rounded-t-xl" src="${cours.photoCouverture}" frameborder="0"></iframe>
...
<div class="p-4">
  <div class="mb-2">
    <h3 class="text-lg font-semibold line-clamp-1">${cours.titre}</h3>
  </div>
...
<div class="flex flex-wrap gap-1 mb-2">
  ${tags.map((tag) => {
    return `<span class="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded">${tag}</span>`;
  }).join('')}
</div>
  ...
  <div class="space-y-1.5 mb-4">
    <div class="flex items-center text-gray-500 text-sm">
      ...
      <span> ${cours.duree}h</span>
    </div>
    <div class="flex items-center text-gray-500 text-sm">
      <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
      </svg>
      <span> ${cours.nomberChapitre}chapitres</span>
    </div>
  ...
  <div class="flex justify-between items-center">
    <span class="text-blue-600 font-bold">${cours.prix}€</span>
    <a  href=" ./etudiant/details.php?id=${cours.id} 
